...
def call_gemini_api(text, width_m, height_m, location, api_key=GEMINI_API_KEY):
    ...
    "Your job is to carefully evaluate the billboard text, its dimensions, and its location, "
    "and determine if it is appropriate and allowed for display. Respond ONLY in valid JSON format.\n\n"
    
    "⚠️ STRICT FORMAT RULES:\n"
    "- Return a JSON object with exactly two keys: \"result\" and \"reason\".\n"
    "- \"result\" must be true, false, or null.\n"
    "- \"reason\" must be a multiline string (lines separated by '\\n').\n"
    "- Do not prefix lines with numbering like 'Line 1:' or '1.'; produce natural sentences only.\n"
    "- Include all relevant analysis you would normally provide, including content, audience, and location evaluation.\n"
    "- After your full analysis, add EXACTLY TWO FINAL LINES (each on its own line):\n"
    f"    Dimensions: {width_m:.2f} m (W) x {height_m:.2f} m — Allowed in this region.\n"
    f"    Location: {location} — Approval status must be verified with local regulations.\n"
    
    "CONTENT ANALYSIS RULES:\n"
    "- Evaluate whether the billboard content is appropriate for the surroundings.\n"
    "- Check for offensive language, adult content, political or religious sensitivity, or misleading claims.\n"
    "- Treat minor slang, humorous, or ironic language as acceptable if the probability of offense is <= 0.5.\n"
    "- Mention any ambiguity or potential misinterpretation in the reason.\n"
    "- Do not ignore the context of the audience (e.g., students, residents, traffic).\n"
    "- Consider cultural norms and local sensitivities.\n"
    
    "LOCATION AND DIMENSION RULES:\n"
    "- Evaluate whether the billboard is in a location where outdoor advertisements are likely permitted.\n"
    "- If location regulations are unclear, explicitly state that approval is unknown.\n"
    "- Dimensions should be included in the final JSON lines, as shown above.\n"
    "- Roads, educational institutions, hospitals, and residential zones require special attention.\n"
    
    "OCR AND TYPO RULES:\n"
    "- Ignore minor spelling errors due to OCR; assume the intended correct text.\n"
    "- Do not let typos change the content analysis.\n\n"
    
    "OUTPUT RULES:\n"
    "- Return only the JSON object; do not include markdown, code fences, or explanations.\n"
    "- Ensure the 'reason' includes natural sentences describing all your analysis steps.\n"
    "- The final two lines must match the exact format for dimensions and location.\n"
    ...
    '{ "result": true, "reason": "The billboard text is humorous and not offensive. Minor slang words are used but are likely interpreted lightly by the audience. The surrounding area is a college, and students are likely to understand the tone as ironic or playful. There is a small probability of misinterpretation (<0.5), but the overall risk is low.\\nDimensions: 7.16 m (W) x 2.54 m (H) — Allowed in this region.\\nLocation: Chennai Institute Of Technology, Kodambakkam – Sriperumbudur Road, Daravur, Sriperumbudur, Kanchipuram, Tamil Nadu, 602117, India — Approval status must be verified with local regulations.\\n" }'
)
...
# Expose only the main function for external call
__all__ = ["process_billboard"]
